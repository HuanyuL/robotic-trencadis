...
# ---- Import directory (EDIT THIS PATH) ----
import_directory = r"F:\\07. IAAC_Internship\\00. PROJECT 01_deco2_robotic_mosaic\\ICP\\ICP_1st Trial\\test05-1\\Merged point clouds"
filename = "merged01.ply"

# ---- Export directory (EDIT THIS PATH) ----
export_directory = r"F:\\07. IAAC_Internship\\00. PROJECT 01_deco2_robotic_mosaic\\ICP\\ICP_1st Trial\\test05-1\\Exported data"
...
def detect_multiple_planes(
    pcd, distance_threshold=0.02, ransac_n=3, num_iterations=1000, min_points=1000, max_planes=5
):
    """
...
def cluster_colored_plane(
    pcd_plane, eps=0.02, min_samples=350, color_weight=1.7, position_weight=0.4
):
    """
...
def group_clusters_by_color(
    ...
    sigma = F.std(axis=0, keepdims=True) + EPS_NUM
    Fz = (F - mu) / sigma
    ...
    report.append(
        f"Mode: {mode} | eps(z): {eps_z:.2f} | min_size: {min_size} | hull_mask: {use_hull_mask}"
    )
    ...
    for g in sorted(set(labels)):
        ...
        report.append(
            f"- Group {g}: count={len(idx)}  meanRGB=({rgb[0]:.2f},{rgb[1]:.2f},{rgb[2]:.2f})  members={idx}"
        )
    ...
    if centroids_out is not None:
        centroids_out.clear()
        centroids_out.extend(centroids)
...
class SegmentationApp:
    def __init__(self, pcd):
        ...
        self.last_report_lines = [
            "Color Grouping Report",
            "Use 'Save Report (.txt)' after grouping.",
        ]
        ...
        self.min_z = float(z_vals.min())
        self.max_z = float(z_vals.max())
        ...
        # Materials
        self.mat_points = rendering.MaterialRecord()
        self.mat_points.shader = "defaultUnlit"
        self.mat_points.point_size = 2.5
        ...
        self.group_eps_slider = gui.Slider(gui.Slider.DOUBLE)
        self.group_eps_slider.set_limits(0.1, 3.0)
        self.group_eps_slider.double_value = 1.0
        self.group_min_slider = gui.Slider(gui.Slider.INT)
        self.group_min_slider.set_limits(1, 50)
        self.group_min_slider.int_value = 1
        self.group_use_hull_checkbox = gui.Checkbox("Use 2D hull colors only")
        self.group_use_hull_checkbox.checked = True

        # Buttons
        self.btn_group_by_color = gui.Button("Group Clusters by Color")
        self.btn_group_by_color.set_on_clicked(self.group_clusters_color_action)
        self.btn_reset_colors = gui.Button("Reset Display Colors")
        self.btn_reset_colors.set_on_clicked(self.reset_display_colors)
        self.btn_save_report = gui.Button("Save Report (.txt)")
        self.btn_save_report.set_on_clicked(self.save_report_to_txt)

        # Centroid buttons
        self.btn_show_centroids = gui.Button("Show Centroids (2D hull)")
        self.btn_show_centroids.set_on_clicked(self.show_centroids)
        self.btn_hide_centroids = gui.Button("Hide Centroids")
        self.btn_hide_centroids.set_on_clicked(self.hide_centroids)
        ...
        # Defaults / limits
        self.slider_min.set_limits(self.min_z, self.max_z)
        self.slider_max.set_limits(self.min_z, self.max_z)
        self.slider_min.double_value = self.min_z
        self.slider_max.double_value = self.max_z
        ...
        self.voxel_slider.set_limits(0.001, 0.05)
        self.voxel_slider.double_value = 0.005
        self.std_slider.set_limits(0.5, 5.0)
        self.std_slider.double_value = 2.0
        self.color_slider.set_limits(0.1, 10.0)
        self.color_slider.double_value = 1.7
        self.pos_slider.set_limits(0.1, 10.0)
        self.pos_slider.double_value = 0.4
        self.eps_slider.set_limits(0.001, 0.1)
        self.eps_slider.double_value = 0.02
        self.min_samples_slider.set_limits(10, 2000)
        self.min_samples_slider.int_value = 350
        self.dist_thresh_slider.set_limits(0.001, 0.1)
        self.dist_thresh_slider.double_value = 0.02
        self.ransac_n_slider.set_limits(3, 10)
        self.ransac_n_slider.int_value = 3
        self.ransac_iter_slider.set_limits(100, 5000)
        self.ransac_iter_slider.int_value = 1000

        self.plane_index_slider.set_limits(0, 0)
        self.plane_index_slider.int_value = 0
        self.cluster_index_slider.set_limits(0, 0)
        self.cluster_index_slider.int_value = 0

        # Workflow buttons
        self.btn_filter = gui.Button("Apply Z Filter")
        self.btn_filter.set_on_clicked(self.apply_z_filter)
        self.btn_preprocess = gui.Button("Preprocess")
        self.btn_preprocess.set_on_clicked(self.preprocess)
        self.btn_segment = gui.Button("Detect Planes")
        self.btn_segment.set_on_clicked(self.detect_planes)
        self.btn_show_plane = gui.Button("Show Selected Plane")
        self.btn_show_plane.set_on_clicked(self.show_selected_plane)
        self.btn_cluster_all = gui.Button("Cluster All Planes")
        self.btn_cluster_all.set_on_clicked(self.cluster_planes_all)
        self.btn_cluster_selected = gui.Button("Cluster Selected Plane")
        self.btn_cluster_selected.set_on_clicked(self.cluster_selected_plane)

        self.btn_hull_selected = gui.Button("Hull 3D: Selected Cluster")
        self.btn_hull_selected.set_on_clicked(self.compute_hull_selected)
        self.btn_hull_all = gui.Button("Hull 3D: All Clusters")
        self.btn_hull_all.set_on_clicked(self.compute_convex_hulls_all)
        self.btn_hull_selected_2d = gui.Button("Hull 2D: Selected Cluster")
        self.btn_hull_selected_2d.set_on_clicked(self.compute_hull_selected_2d)
        self.btn_hull_all_2d = gui.Button("Hull 2D: All Clusters")
        self.btn_hull_all_2d.set_on_clicked(self.compute_convex_hulls_all_2d)
    ...
    def apply_z_filter(self):
        ...
        """
        z0 = self.slider_min.double_value
        z1 = self.slider_max.double_value
        if z0 > z1:
            z0, z1 = z1, z0
        ...
        self.show_geometry(
            self.filtered_pcd if self._has_points(self.filtered_pcd) else self.original_pcd
        )
    ...
    def preprocess(self):
        ...
        """
        if not self._has_points(self.filtered_pcd):
            return
        voxel = self.voxel_slider.double_value
        std = self.std_slider.double_value
        self.processed_pcd = preprocess_point_cloud(
            self.filtered_pcd, voxel_size=voxel, std_ratio=std
        )
    ...
    def detect_planes(self):
        ...
        """
        if not self._has_points(self.processed_pcd):
            return
        ...
        self.plane_index_slider.set_limits(0, max_plane_idx)
        self.plane_index_slider.int_value = 0
        self.selected_plane_idx = 0
        ...
        if self._has_points(self.remaining_cloud):
            ...
            grey.paint_uniform_color([0.7, 0.7, 0.7])
            display.append(grey)
        self.show_geometry(display)
        # Update report buffer (not shown, used for saving)
        self.last_report_lines = [
            "Color Grouping Report",
            "Planes detected. Cluster and then group to generate a report.",
        ]
    ...
    def cluster_planes_all(self):
        ...
        for i, plane in enumerate(self.planes_cluster):
            clusters_disp, clusters_orig = cluster_colored_plane(
                plane, eps, min_samples, color_weight, pos_weight
            )
        ...
        self.last_report_lines = [
            "Color Grouping Report",
            "Clusters ready. Run 'Group Clusters by Color' then Save Report.",
        ]
    ...
    def cluster_selected_plane(self):
        ...
        pos_weight = self.pos_slider.double_value
        clusters_disp, clusters_orig = cluster_colored_plane(
            self.planes_cluster[idx], eps, min_samples, color_weight, pos_weight
        )
        ...
        self.cluster_index_slider.set_limits(0, max_idx)
        self.cluster_index_slider.int_value = 0
        self.selected_cluster_idx = 0
    ...
    def compute_hull_selected_2d(self):
        ...
        if ls is None:
            print("Could not compute 2D hull (cluster too small).")
            return
    ...
    def reset_display_colors(self):
        ...
        display = list(self.clusters)
        if self.hull_lines:
            display += self.hull_lines
        if self.hull_lines_2d:
            display += self.hull_lines_2d
    ...
    def save_report_to_txt(self):
        ...
        path = os.path.join(os.getcwd(), f"color_group_report_{ts}.txt")
        try:
    ...
    # ---- NEW: Export Selected Cluster (.json) ----
    def export_selected_cluster_json(self):
        ...
        orig_cluster = (
            ...
            else self.clusters[cidx]
        )

        # plane frame, hull, centroid
        poly2, poly3, origin, u, v, n, centroid3d = compute_hull_poly2_poly3(orig_cluster)

        # ---- NEW: color grouping per-cluster mapping ----
        cg = None
        if isinstance(self.color_group_labels, np.ndarray) and len(self.color_group_labels) == len(
            self.clusters
        ):
            g_id = int(self.color_group_labels[cidx])
            if g_id != -1:
                cg = {"group_id": g_id, "group_mean_rgb": self.color_group_mean_rgb.get(g_id, None)}
            else:
    ...
    def export_all_clusters_json(self):
        ...
        have_groups = isinstance(self.color_group_labels, np.ndarray) and len(
        ...
        for cidx, disp_cluster in enumerate(self.clusters):
            orig_cluster = (
                self.cluster_originals[cidx] if cidx < len(self.cluster_originals) else disp_cluster
            )
            poly2, poly3, origin, u, v, n, centroid3d = compute_hull_poly2_poly3(orig_cluster)
